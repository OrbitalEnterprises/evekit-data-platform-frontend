...
var ServiceError = function(opt_json) {
  opt_json = opt_json || {};
...
function handleRemoteResponse(response) {
  if (angular.isDefined(response.obj) &&
      response.obj != null)
    throw new ServiceError(response.obj);
  throw new ServiceError({errorCode: 0, errorMessage: "internal service error"});
}

/** Utilities to handle server interactions. */
(function(){
var ekdpRemoteServices = angular.module('ekdpRemoteServices', []);

/**
 * Service for retrieving swagger endpoints for the proxy.  Returns a promise
 * that resolves to the client when it's ready.
 */
ekdpRemoteServices.factory('SwaggerService', [
  function() {
